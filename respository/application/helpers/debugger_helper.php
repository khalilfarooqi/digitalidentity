...
function vd($arr, $is_bool = TRUE, $title = '')
{
	if ($title) {
		echo '<strong>' . $title . '</strong>';
	}
 ...
 * glq()
 * -----------------------------------------
 * The dd function dumps the last query execution in database:
 *
 */
function getLastQuery($conn = null, $is_bool = TRUE, $title = '')
{
    if( $conn ) {
        
        dd( $conn->last_query(), $is_bool, $title );

        return;
    }

	dd( get_instance()->db->last_query(), $is_bool, $title );
}
